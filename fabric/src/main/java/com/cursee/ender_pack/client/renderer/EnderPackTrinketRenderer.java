...
public class EnderPackTrinketRenderer implements TrinketRenderer {
    ...
    public void render(ItemStack itemStack, SlotReference slotReference, EntityModel<? extends LivingEntity> entityModel, PoseStack poseStack, MultiBufferSource multiBufferSource, int light, LivingEntity entity, float v, float v1, float v2, float v3, float v4, float v5) {
        ...
        if (crouching) poseStack.mulPose(new Matrix4f().setRotationXYZ(30.0f * (Mth.PI/180.0f), 0, 0));
        else poseStack.mulPose(new Matrix4f().setRotationXYZ(0, 0, 0));
        ...
        model.renderToBuffer(poseStack, multiBufferSource.getBuffer(model.renderType(EnderPackClient.ENDER_PACK_TEXTURE_LOCATION)), light, OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
        poseStack.popPose();
    }
}
